...
def round_EV_scheduling_fixed_interval(ins, guess_x={}, round_x_after_y=True):
    t1 = time.time()
    sol = max_ev_scheduling_OPT_fixed_interval(ins, fractional=True)
    print(sol.x)
    print(sol.obj)
    frac_sol = copy.deepcopy(sol)
    sol.frac_sol = frac_sol
    ...
    if sol.succeed:
        ...
        counting = 0
        rounded_solution_feasibility = 0
        the_best = copy.deepcopy(sol)
        rounded_sol = copy.deepcopy(sol)
        the_best.obj = 0
        while not rounded_solution_feasibility or counting < 50:
            sol = max_ev_scheduling_OPT_fixed_interval(ins, fractional=True)
            frac_sol = copy.deepcopy(sol)
            sol.frac_sol = frac_sol
            sol.frac_x_comp_count = 0
            ...
            # customers_heavy = []
            customers_light = []
            # dividing customers into heavy and light.

            for m in customers:
                #     if W1[m][m] > delta * 1000000 * 1000000:
                #         customers_heavy.append(m)
                #     else:
                customers_light.append(m)
                # print(customers_heavy)
                # print(customers_light)
                # print(len(customers_heavy))
                # print(len(customers_light))
            #
            #         # putting heavy customers into the enumeration set.
            #         # print(sol.x)
            #         enumeration_set = []
            #         # max_guessed_set_size = int(1.0 / delta)
            #         max_guessed_set_size = 0.1 * customers
            #         counter = 0
            #         for m in customers:
            #             # rounding y
            #             if m in customers_heavy:
            #                 counter = counter + 1
            #                 if counter <= max_guessed_set_size:
            #                     enumeration_set.append(m)
            #
            # randomized rounding. #for the light only
            # print(sol.x)
            for m in customers:
                if m in customers_light:
                    print m
                    max_array = {c: [] for c in ins.customer_charging_options[m]}
                    for c in ins.customer_charging_options[m]:
                        max_array[c].append(sol.x[m, c])
                        # print(max_array)
                    max_array = sorted(max_array.items(), key=operator.itemgetter(1))

                    # print(sol.x)
                    print(max_array)
                    for i in max_array:
                        print i
                        print sol.x[m, int(i[0])]
                        if 0.0001 < sol.x[m, int(i[0])] < 1 - 0.0001:
                            print float(i[1][0])
                            ...
                            # print(alpha)
                            # print(rand)
                            if rand <= alpha_delta * float(i[1][0]):  # ceiling
                                # print(alpha * i)
                                print("ceil")
                                sol.x[m, int(i[0])] = 1
                                print i
                                ...
                                sol.x[m, int(i[0])] = 0
                        elif sol.x[m, int(i[0])] >= 1 - 0.0001:
                            print "too big"
                            sol.x[m, int(i[0])] = 1
                            break
                        elif sol.x[m, int(i[0])] <= 0.0001:
                            print "too small"
                            sol.x[m, int(i[0])] = 0
                    print sol.x
            for ss in customers:
                for k in ins.customer_charging_options[ss]:
                    if 0.0001 < sol.x[ss, k] < 1 - 0.0001:
                        print ss
                        print k
                        sol.x[ss, k] = 0
            ...
            #
            #         # print(sol.x)
            z = [] * int(ins.n * len(ins.charging_rates))
            X1 = [] * int(ins.n * len(ins.charging_rates))
            for m in np.arange(ins.n):
                for c in ins.customer_charging_options[m]:
                    z.append(ins.customer_utilities[m])
                    X1.append(sol.x[m, c])
            obj1 = np.dot(np.transpose(z), X1)

            # #         # print(sol.x)
            # #         # print(rounded_sol.x)
            rounded_solution_feasibility = u.check_ev_scheduling_fixed_int_sol_feasibility(ins, sol,
                                                                                           fractional_sol=False)
            print ("the best")
            print(the_best.obj)
            if obj1 > the_best.obj:
                the_best = sol
                the_best.obj = obj1
            #
            #         print("obj1")
            ...
            #     # for size in np.arange(1, max_guessed_set_size.any() + 1):
            #     #     for guess_I1 in itertools.combinations(enumeration_set, size):
            #     #         obj2 = 0
            #     #         # print(guess_I1)
            ...
            #     #         if obj2 > max_obj2:
            #     #             max_obj2 = obj2
            #     #         max_obj2 = max_obj2 + max_obj2
            #     # print(max_obj2)
            #     max_obj2 = 0
        obj = max(the_best.obj, 0)
        sol.running_time = time.time() - t1

        # calculate P
        P = {}
        for t in np.arange(ins.scheduling_horizon):
            P[t] = ins.base_load_over_time_P[t]
        for (m, c) in ins.customers_at_time[t]:
            P[t] += sol.x[(m, c)] * ins.charging_rates[c]
...
def max_ev_scheduling_OPT_fixed_interval(ins, guess_x={}, fractional=False, debug=False, tolerance=0.001,
                                         alg="min_OPF_OPT"):
    t1 = time.time()
    A = gbp.Model("max_ev_scheduling_OPT_fixed_interval")
    ...
    for k in np.arange(ins.n):
        for c in ins.customer_charging_options[k]:
            if fractional:
                x[(k, c)] = A.addVar(vtype=gbp.GRB.CONTINUOUS, name="x[(%d,%d)]" % (k, c))
            else:
                x[(k, c)] = A.addVar(vtype=gbp.GRB.BINARY, name="x[(%d,%d)]" % (k, c))

    for t in time_path:
        P[t] = A.addVar(vtype=gbp.GRB.CONTINUOUS, name="P_%s" % str(t))

    for t in np.arange(192):
        # Define W for each time-slot.
        X1 = [] * int(ins.n * len(ins.charging_rates))
        for (m, c) in ins.customers_at_time[t]:
            #print (m, c)
            X1.append(x[m, c])

        # matrix W containing the CN*CN
        # print t
        # print X1
        # print ins.charging_rates
        # print ins.base_load_over_time_P[t]
        # print ins.base_load_over_time_Q[t]
        w1, h1 = len(X1), 2
        Q = [[0 for O in range(w1)] for y in range(h1)]
        w2, h2 = len(X1), len(X1)
        W1 = [[0 for O in range(w2)] for y in range(h2)]

        if len(ins.customers_at_time[t]) != 0:
            z = -1
            for (m, c) in ins.customers_at_time[t]:
                z = z + 1
                #print x[m, c]
                Q[0][z] = ins.charging_rates[c] + ins.base_load_over_time_P[t]
                Q[1][z] = ins.base_load_over_time_Q[t]
       # print Q
        W1 = np.dot(np.transpose(Q), Q)
        #print W1
        d = np.diagonal(W1)
        d = np.transpose(d)
        d = sum(d)
        #print d
        d = gbp.quicksum(d * x[m, c] for (m, c) in ins.customers_at_time[t])
        #print d
        A.addConstr(d, gbp.GRB.LESS_EQUAL, ins.capacity_over_time[t])
        #print ins.capacity_over_time[t]

        # THE power constraint.

        p_sum = gbp.quicksum(
            [x[m, c] * ins.charging_rates[c] for m in np.arange(ins.n) for c in ins.customer_charging_options[m] if
             ins.customer_charge_start_at_time[m] <= t < ins.customer_charge_start_at_time[m] +
             ins.customer_charging_length[k] + 1])
        A.addQConstr(P[t], gbp.GRB.EQUAL, p_sum)  # power flow equations
        Q1 = ins.base_load_over_time_Q[t]
        A.addQConstr(P[t] * P[t] + Q1 * Q1, gbp.GRB.LESS_EQUAL,
                     ins.capacity_over_time[t], name="gen_capacity_%d" % t)
        for k in np.arange(ins.n):
            # single charging option per customer contraints
            X = gbp.quicksum([x[(k, c)] for c in ins.customer_charging_options[k]])
            A.addLConstr(X, gbp.GRB.LESS_EQUAL, 1, "sum_c X(%d) <= 1" % k)
            num_of_constraints += 1
            num_of_constraints_paper_formulation += 1

    z = [] * int(ins.n * len(ins.charging_rates))  # Utility matrix for each of the CN
    X1 = [] * int(ins.n * len(ins.charging_rates))  # X to store CN values.
    ##Filling in the matricies .
    for k in np.arange(ins.n):
        for c in ins.customer_charging_options[k]:
            z.append(ins.customer_utilities[k])
            X1.append(x[k, c])

        for c in ins.customer_charging_options[k]:
            A.addLConstr(x[(k, c)] >= 0, "x[%s]: lb" % str((k, c)))
            num_of_constraints += 1
            num_of_constraints_paper_formulation += 1

            A.addConstr(x[(k, c)] <= 1, "x[%s]: ub" % str((k, c)))
            num_of_constraints += 1
            num_of_constraints_paper_formulation += 1

    # computing the objective.
    obj = np.dot(np.transpose(z), X1)
    A.setObjective(obj, gbp.GRB.MAXIMIZE)
    A.update()
    #
    # for t in time_path:
    #     # W matrix
    ...
    #         Q[0][k] = 7000.0  # real value of S.
    #         Q[1][k] = 0.0  # Imaginary value S.
    #     W1 = np.dot(np.transpose(Q), Q)
    #     rhs_P = np.dot(np.transpose(X1), W1)  # X^T . W
    #     rhs_P = np.dot(rhs_P, X1)  # X^T . W . X
    #     rhs_P = rhs_P + ins.base_load_over_time_P[t]
    #     m.addQConstr(rhs_P, gbp.GRB.LESS_EQUAL, ins.capacity_over_time[t], "C_%d" % t)
    #     num_of_constraints += 2
    #     num_of_constraints_paper_formulation += 1

    #     # Second constraint (diagonal)
    #     d = np.diagonal(W1)
    #     d = np.dot(np.transpose(d), X1)
    #     # print(d)
    #     m.addLConstr(d, gbp.GRB.LESS_EQUAL, ins.capacity_over_time[t])
    #

    # ins.charging_stations_chargers = [150, 100, 75]

    # for c in np.arange(3):
    #     # single charging option per customer contraints
    #     Z = gbp.quicksum(x[(k, c)] for k in np.arange(ins.n) if c in ins.customer_charging_options[k])
    #     m.addLConstr(Z, gbp.GRB.LESS_EQUAL, ins.charging_stations_chargers[c], "sum_c X(%d) <= 1" % k)
    #     num_of_constraints += 1
    #     num_of_constraints_paper_formulation += 1
    ...
    # sol.gurobi_model = m
    #
    if u.gurobi_handle_errors(A, algname=alg):
...
def max_ev_scheduling_OPT_fixed_interval_not_relaxed(ins, guess_x={}, fractional=False, debug=False, tolerance=0.001,
                                                     alg="min_OPF_OPT"):
    t1 = time.time()
    ...
    for t in time_path:
        ...
        for (k, c) in ins.customers_at_time[t]:
            z = [] * int(ins.n * len(ins.charging_rates))
            X1 = [] * int(ins.n * len(ins.charging_rates))
            # print("I'm in the obj loop")
            # print(k)
            # print(c)
            z.append(ins.customer_utilities[k])
            X1.append(x[k, c])
            obj = obj + np.dot(np.transpose(z), X1)
    # print(obj)
    print(obj)
    m.setObjective(obj, gbp.GRB.MAXIMIZE)
    m.update()
    ...
    for t in time_path:
        ...
        # print(W1)
        # print(len(W1))
        # ev_charge_list = [x[(k, c)] * (ins.charging_rates[c]) for (k, c) in ins.customers_at_time[t] if
        # len(ins.customers_at_time[t]) != 0]
        # print(ins.customers_at_time) ins.base_load_over_time[t] +
        rhs_P = np.dot(np.transpose(X1), W1)
        rhs_P = np.dot(rhs_P, X1)
        rhs_P = rhs_P + ins.base_load_over_time[t]
        # print(rhs_P)
        # print(rhs_P)
        # m.addConstr(P[t], gbp.GRB.EQUAL, rhs_P, "P_%d" % t)
        m.addConstr(rhs_P, gbp.GRB.LESS_EQUAL, ins.capacity_over_time[t], "C_%d" % t)
        # print(ins.capacity_over_time[t])
        num_of_constraints += 2
        num_of_constraints_paper_formulation += 1
        ...
        for k in np.arange(ins.n):
            ...
            for c in ins.customer_charging_options[k]:
                ...
                m.addConstr(x[(k, c)] <= 1, "x[%s]: ub" % str((k, c)))
                num_of_constraints += 1
                num_of_constraints_paper_formulation += 1
