...
def gurobi_handle_errors(m, algname=''):
    if m.status != gbp.GRB.status.OPTIMAL:
        ...
        if (m.status == 3):
            ...
            logging.warning(
                " %s: SUBOPTIMAL 13	Unable to satisfy optimality tolerances; a sub-optimal solution is available." % algname)
        elif m.status == 12:
            logging.warning(
                " %s: NUMERIC: Optimization was terminated due to unrecoverable numerical difficulties." % algname)
            return False
...
def print_instance(ins, detailed=True):
    ...
    print(
            ' z : %s' % {e: (round(ins.topology[e[0]][e[1]]['z'][0], 3), round(ins.topology[e[0]][e[1]]['z'][1], 3)) for
                         e
                         in
                         ins.topology.edges()})
...
def check_ev_scheduling_fixed_int_sol_feasibility(ins, sol, fractional_sol=False):
    ...
    for k1 in np.arange(ins.n):
        for c in ins.customer_charging_options[k1]:
            z.append(ins.customer_utilities[k1])
            X1.append(sol.x[k1, c])

    for t1 in time_path:
        # W matrix
        X1 = [] * int(ins.n * len(ins.charging_rates))
        for (m, c) in ins.customers_at_time[t1]:
            print (m, c)
            X1.append(sol.x[m, c])
        ...
        if len(ins.customers_at_time[t1]) != 0:
            z = -1
            for (m, c) in ins.customers_at_time[t1]:
                z = z + 1
                #print x[m, c]
                Q[0][z] = ins.charging_rates[c] + ins.base_load_over_time_P[t1]
                Q[1][z] = ins.base_load_over_time_Q[t1]
        #print Q
        W1 = np.dot(np.transpose(Q), Q)
        rhs_P = np.dot(np.transpose(X1), W1)  # X^T . W
        rhs_P = np.dot(rhs_P, X1)  # X^T . W . X
        assert (rhs_P <= (
                ins.capacity_over_time[t1] - ins.base_load_over_time_P[t1])), "capacity violated at time %d" % t1
    energy_usage = {}
    for k in range(ins.n):
        energy_usage[k] = 0
        sum_of_x = 0
        for c in ins.customer_charging_options[k]:
            ...
            for t in ins.customer_charging_time_path[k]:
                energy_usage[k] += sol.x[(k, c)] * ins.charging_rates[1] * ins.step_length
                if not fractional_sol:
                    assert sol.x[(k, c)] in [1, 0], "x[%d,%d] is fractional" % (k, c)
        assert sum_of_x <= 1, "sum X > 1! %f" % sum_of_x
        if not fractional_sol:
            assert (energy_usage[k] <= ins.customer_usage[k] and sum_of_x == 0) or (
                    energy_usage[k] >= ins.customer_usage[k] and sum_of_x == 1), "x_c inconsistant"
            # print "satisfy ratio %d:"%k, energy_usage[k]/ins.customer_usage[k]

    for t in time_path:
        number_chargers = [0, 0, 0]
        if t in ins.customer_charging_time_path[k]:
            for c in ins.customer_charging_options[k]:
                number_chargers[c] = number_chargers[c] + sol.x[k, c]
        assert (number_chargers[0] <= 5 and number_chargers[1] <= 5 and number_chargers[
            2] <= 5), "maximum number of chargers violation"
...
def check_ev_scheduling_sol_feasibility(ins, sol, fractional_sol=False):
    ...
    for t in np.arange(ins.scheduling_horizon):
        ...
        assert (total_ev_charge_at_time[t] - ins.rounding_tolerance <= (
                ins.capacity_over_time[t] - ins.base_load_over_time[t])), "capacity violated at time %d" % t
    ...
    for k in range(ins.n):
        ...
        if not fractional_sol:
            assert (energy_usage[k] <= ins.customer_usage[k] and sol.y[k] == 0) or (
                    energy_usage[k] >= ins.customer_usage[k] and sol.y[k] == 1), "y and x are inconsistant"
        # print "satisfy ratio %d:"%k, energy_usage[k]/ins.customer_usage[k]
