...
def sim_instance_ev_scheduling_fixed_intervals(scenario="L", n=10,
    ...
    ins.charging_stations_chargers = [100, 100, 100]
    # ins.charging_rates_stds = charging_rates_stds
    # load and supply profile
    ins.capacity_over_time = np.ones(int(time_steps / step_length)) * 1000000 * 1000000
    ...
    value_q = math.acos(0.9)
    value_q = math.sin(value_q)
    ins.base_load_over_time_P = 1000 * 0.9 * num_base_load_household * np.repeat(two_days, 1 / step_length)
    ins.base_load_over_time_Q = 1000 * value_q * num_base_load_household * np.repeat(two_days, 1 / step_length)
